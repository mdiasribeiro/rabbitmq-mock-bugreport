...
class TestSuite extends TestKit(ActorSystem("TestSuite")) with ImplicitSender with FunSuiteLike {
  ...
  test("Consumer should not require rebinding") {
    ...
    // The following line should not be required, but with it the test works (only works on 1.0.12 SNAPSHOT)!
    // With the old version, the message wouldn't be delivered no matter what.
    bindActorToQueue(mockRmqChannel, sink.ref, "queue")
